...
## 스트링 리터럴과 new String()
- 레퍼런스 변수는 스택에 할당되는데 스트링은 static data, new String()은 힙에 할당이 됨.
- 스트링 객체는 수정이 불가능하다. 스트링을 비교시 equals()를 이용해야한다.

## 제네릭과 컬렉션
- 컬렉션
- 요소라고 불리는 객체들을 담는 저장소
- 요소의 개수에 따라 크기 자동 조절. 요소의 삽입, 삭제에 따른 요소의 위치 자동 이동
- 배열과 유사하나 담을 수 있는 요소의 개수가 가변. 담아둔 객체들의 삽입, 삭제 검색 등 관리 용이
- 제너릭
- 다양한 타입의 요소를 담기 위해 컬렉션을 제네릭 기법으로 구현한다.
- 특정 타입만 다루지 않고 여러 종류의 타입으로 변신할 수 있도록 클래스나 메소드를 일반화시키는 기법이다.
- 사용 시 특정 타입으로 구체화해야함.
- 모든 종류의 데이터 타입을 다룰기 위해 일반화된 타입 매개 변수로 클래스나 메소드를 작성해야한다.
1) Vector<E>
  + 여러 객체들을 삽입, 삭제, 검색하는 컨테이너 클래스이다.
  + 객체만 삽입이 가능하다.
  + 임의의 위치 또는 끝에 객체를 추가할 수 있다.
  + 임의의 위치에 있는 객체 삭제 가능하고 삭제 후 자동 자리 이동이 된다.
- 컬렉션에서 그냥 integer를 넣어도 객체로 알아서 바꿔서 즉 자동 박싱/언박싱 기능을 지원해준다.
2) ArrayList<E>
  + 객체만 삽입가능하다.
  + 삽입, 삭제 역시같다.
  + 벡터와 달리 쓰레드 동기화 기능 지원하지 않음.
  + 다수의 쓰레드가 ArrayList에 접근할 때 충돌 발생.
  + 멀티 쓰레드 프로그램이 아니라면 가벼운 ArrayList 사용 권장.
3) LinkedList<E>
  + 객체들이 양방향으로 연결되어 관리되고 나머지는 유사함.
  + 성능이 가장 떨어질 수 밖에 없음. 메모리 활용이 좋은 대신 삽입, 삭제할 때마다 링크를 다시 연결하기 때문이다.
4) Iterator <E>
  + 이 놈위에 포함되어 있음. 리스트 구조의 컬렉션에서 요소의 순차 검색을 위해 사용함.
  + iterator()를 호출하면 Iterator 객체가 반환되고 인덱스로 순차 검색하는 것보다 훨씬 빠름.
5) HashMap<K,V>
  + 키, 값을 한쌍으로 삽입함.
...
## 추상화
> 컨텍스트와 관련없는 정보는 숨기거나 관련된 정보만 알 수 있도록 하는 것
1) 데이터 추상화 : 복잡한 데이터 형태를 생성하기 위해서 작은 데이터 타입들을 사용하는 방법을 의미.
2) 제어 추상화 : 어떤 클래스의 메소드를 사용하는 사용자에게 해당 메소드의 작동방식과 같은 로직을 숨기기 위함. 만일 메소드 내 로직이 변경되어도 실제 사용자는 변경된 내용이 어떤 것인지 알 필요 없이 이전과 동이랗게 메소드를 사용할 수 있음. 따라서 로직이 바뀌어도 사용자에게 영향 x
## 캡슐화
> 관련있는 변수, 메소드를 하나의 클래스로 묶고 외부에서 쉽게 접근하지 못하도록 은닉하는 것  
> 객체의 직접적인 접근을 막고 외부에서 내부의 정보에 직접 접근 및 변경이 불가하고 객체가 제공하는 필드와 메소드를 통해서만 접근이 가능하다. 정보은닉과 구현 은닉을 모두 갖고 있다.
1) 정보은닉의 방법으로는 접근 제어자를사용하여 외부에서 접근할 수 없도록 하며 인터페이스를 통해 구현은닉을 달성하는 것이다.
2) 구현은닉은 객체가 책임을 이행하는 방식을 수정할 수 있도록 개발자에게 자유를 제공한다. 이는 설계가 변경될 때 유용하다.
## 상속
> 하나의 클래스가 슈퍼클래스의 속성과 행동을 얻게 되는 방법.
- 코드의 재사용성과 유지보수를 위해 사용된다.
2) 서브 클래스는 슈퍼 클래스의 non-private 멤버들을 상속 받게되며 생성은 멤버가 아니기 때문에 상속되지 않음.
## 다형성
> 같은 자료형에 여러가지 객체를 대입하여 다양한 결과를 얻어내는 성질을 의미.
1) compile time polyporphism : 컴파일러가 필요한 모든 정보를 가지고 있고 프로그램 컴파일 중에 호출할 방법을 알기 때문에 컴파일 시간에 적절한 메소드를 각각의 객체에 바인딩할 수 있다. 정적 바인딩이나 early binding이라고도 불린다. 메소드 오버로딩을 통해 사용된다.
2) runtime polymorphism은 동적 바인딩이라고 불리며 메소드 오버라이딩과 연관있음. 일반적으로 서브 클래스와 슈퍼 클래스가 존재할 때 사용되는데 런타임 과정에서 해당 인스턴스에 맞는 메소드를 호출한다. 

- 객체는 속성과 행동으로 구성되어 있는 하나의 단위이고 궁극적 실체이다.
- 클래스는 객체를 만드는 설계도이고 어느 무리의 객체를 특징 짓는 데에 공통적으로 갖춘 모양을 뽑아 하나의 틀을 만든 것이다. 이러한 과정을 추상화라고 한다.
- 인스턴스는 클래스를 new 명령문으로 메모리에 생성한 객체이다. 설계도를 통해 독립적인 객체를 만든 것이다.
__즉, 우리가 프로그래밍으로 표현하려는 실체는 객체이고, 이러한 실체를 추상화한 설계도는 클래스이며, 클래스를 인스턴스화하여 객체가 생성되는 것이다.__
