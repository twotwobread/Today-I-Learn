...
## 1 스레드의 개념
### 1.1 스레드의 정의
- 안심 스테이크를 만드는 과정에 빗대어 설명
  1. 조리도구 준비
  2. 레시피에 따라 고기, 채소를 굽고 소스
  + 준비과정 = 프로세스 생성 과정, 조리과정 = 스레드
  + 프로세스 = 요리 작업 전체, 스레드 = 조리
- 프로세스의 작업 과정
  1. OS는 코드, 메모리를 메모리로 가져오고 PCB 생성, 필요한 메모리 영역 확보 후 프로세스 준비 큐에 삽입
  2. CPU 스케줄러가 CPU에 전달 -> CPU 실제 작업 수행, 이때 전달하는 작업 하나가 스레드 ( CPU의 처리 작업 단위 스레드 )
  + OS 입장의 작업 단위 = 프로세스, CPU 입장의 작업 단위 = 스레드  
  + 스레드 = 프로세스의 코드에 정의된 절차에 따라 CPU에 작업 요청을 하는 실행 단위.
- 프로세스, 스레드의 구분을 위한 작업의 크기 고려
  + 일이 모여 하나의 작업이 됨.
  + 집짓기 = 토지 구입, 설계, 토목 공사, 인테리어 등, 집짓기(작업) = 토지 구입(일)
  + 스레드 = 일을 처리, 프로세스 = 작업을 처리
  + 작업의 크기 = job(처리) > task(프로세스) > operation(스레드)
    - 여러 스레드가 모여 프로세스를 이루고 여러 프로세스가 모여 처리가 되며, 여러 프로세스를 모아 한번에 처리하는 방법을 일괄 작업(batch job)이라 함
### 1.2 프로세스와 스레드의 차이
> 프로세스끼리는 약하게 연결, 스레드끼리는 강하게 연결
- 프로세스
  + 요리를 예로 생각
  + 코스 C 요리에 안심, 죽, 디저트가 나간다 가정
  + 안심과 죽은 큰 영향을 미치지 않음. ( 프로세스 약하게 연결 )
- 스레드
