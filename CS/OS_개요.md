...
<!--
* 일단 운영체제가 없으면 처음 개발될 때 추가된 기능에서 성능 및 기능을 향상, 추가 시킬 수 없다. O
* OS에서 컴퓨팅 자원 관리를 하는데 그것을 사용자가 직접해야하고 그로 인해 컴퓨팅 자원 손상이 발생할 수 있다. (이건 아닌듯)
-->
...
<!-- 대화형 시스템 X, 시분할 시스템 O , 다중 사용자 시스템 -->
...
### < 심화 문제 >
1. 운영체제의 역할을 설명하시오.
: <!-- 운영체제의 역할은 자원의 관리, 자원의 보호, 사용자 인터페이스 구현, 하드웨어 인터페이스 구현이 있다. -->
<!-- 답) 첫째, 자원관리로 컴퓨터로 키보드, 네트워크 카드, 사운드 카드 등의 컴퓨터 자원을 사용할 때, 운영체재는 자원을 응용 프로그램에 나누어 주어 사욪가 원활하게 작업할 수 있도록 돕느다.
둘째, 비정상적인 작업으로부터 컴퓨터 자원을 보호한다.
셋째, 다양한 장치를 사용할 수 있도록 해주는 하드웨어 인터페이스를 제공한다. 운영체제는 CPU, 메모리, 키보드, 마우스와 같은 다양한 하드웨어를 일관된 방법으로 사용할 수 있도록 지원한다. 하드웨어 인터페이스가 자동으로 설치되게 함으로써 하드웨어의 종류에 상관없이 사용할 수 있게 해준다.
넷째, 사용자 인터페이스를 제공한다. 이는 사용자가 OS를 편리하게 사용하도록 지원하기 위함이다. GUI, 스마트폰 터치 스크린 등이 있다.-->

2. 운영체제의 목표를 설명하시오.
: <!-- 운영체제의 목표는 운영체제의 역할과 연관지어 설명할 수 있다. 자원의 관리는 효율성을 목표로 하고 자원의 보호는 안정성, 사용자 인터페이스 구현은 편리성, 하드웨어 인터페이스 구현은 확장성의 목표를 가지고 있다. -->
<!--
답) 첫째, 효율성이다. 적은 자원을 사용하여 결과를 빨리 보여주는 것이 효율적인 운영체제이다. 같은 자원을 사용하여 더 많은 작업량을 처리하거나, 같은 작업량을 처리하는데 보다 적은 자원을 사용하는 것이다. 일반적으로 운영체제의 크기를 최소화하고 운영체제가 사용하는 코드를 최적화해야한다.
둘째, 안정성이다. 운영체제는 하드웨어 전체를 관리하는 소프트웨어로, 사용자와 응용 프로그램은 운영체제 위에서 작업을 한다 볼 수 있다. 운영체제가 불안정하면 모든 작업이 불안해 질 수 있으므로 안정성이 담보되어야 한다.
셋째, 확장성이다. 운영체제는 다양한 시스템 자원을 추가하거나 제거하기가 편리해야 한다. 하드웨어의 종류에 상관없이 꽂으면 바로 실행할 수 있는 플러그 앤드 플레이 기능을 제공해야한다.
넷째, 편리성이다. 사용자가 편리하게 작업할 수 있는 환경을 제공해야한다. 운영체제 입장에서는 효율성을 높이려면 응용 프로그램이나 사용자에게 제약을 가하면 되지만, 사용자가 불편을 겪기에 다향한 편리성을 제공하면서도 자원의 낭비 요소를 막아야한다.
-->

3. CPU 집중 잡업과 입출력 집중 작업에 대해 설명하시오.
: <!-- 말그대로 CPU 집중 작업은 CPU의 작업에 집중하는 계산기 혹은 일괄 작업 시스템 같이 컴퓨팅에 집중을 하는 작업이다.
입출력 집중 작업은 동영상 플레이어 처럼 출력에만 집중하는 작업과 같이 입출력의 동작에만 집중을 하는 작업이다. -->
<!-- 
답) CPU 집중 작업은 일괄 작업 시스템에서 프로그램이 실행되는 동안에 입출력이 불가능하기 때문에 대부분의 작업이 CPU만 사용하는 계산 작업을 말한다.
입출력 집중 작업은 대화형 시스템에서는 프로그램이 실행되는 동안에 입출력이 가능하기 때문에 입출력 집중 작업이 생겨났다. 입출력 집중 작업 또는 입출력 편향 작업은 대부분의 시간을 주변장치의 입력과 출력에 사용하며, 동영상 플레이어와 데이터 베이스 같은 프로그램을 예로 들 수 있다.
-->

4. 실시간 시스템에 대해 설명하시오.
: <!-- 실시간 시스템은 최소 처리 시간을 보장해주는 시스템이다.
실시간 시스템은 경성 실시간 시스템, 연성 실시간 시스템으로 나뉘는데 핵 미사일 발사와 같이 지연이 절대 허용되지 못하는 경우 경성 실시간 시스템으로 분류되고 연성 실시간 시스템은 실시간 동영상 스트리밍처럼 실시간으로 데이터를 받으면 좋지만 지연이 어느정도 허용되는 경우로 분류한다. -->
<!--
답) 실시간 시스템은 특정 시스템에서 일정 시간 안에 작업이 처리되도록 보장하는 시스템이고 경성 실시간 시스템과 연성 실시간 시스템으로 나뉜다.
경성 실시간 시스템은 지정한 응답 시간을 정확히 지키는 시스템으로 원자력 발전소의 원자로 온도 제어, 미사일 요격과 가틍ㄴ 작업에 이용된다.
연성 실시간 시스템은 지정한 응답 시간을 최대한 지키지만 융통성이 어느 정도 허용된 시스템이다. 예로 동영상 재생기는 응답 시간 안에 작업이 처리되지 않으면 끊김 현상이 발생하지만 치명적인 결과를 낳지는 않는다.
-->

5. 클라우드 컴퓨팅에 대해 설명하시오.
: <!-- 클라우드 컴퓨팅은 컴퓨팅 자원이 구름에 가려 보이지 않는다는 의미이다.
원래는 내가 어떤 환경을 설정하기 위해서는 직접 컴퓨팅 자원들을 사서 나만의 환경을 만들어야 했는데 클라우드 컴퓨팅은 큰 회사나 좋은 컴퓨팅 자원을 가진 사람이 해당 컴퓨팅 자원을 돈을 받고 빌려주는 형태이다. 그래서 컴퓨팅 자원을 빌린 사람은 직접 환경을 구성하지 않고 좋은 환경을 빌려 사용할 수 있는 형태이다.-->
<!-- 
답) 필요한 컴퓨팅 자원만 구매하여 사용하는 컴퓨팅 환경을 그리드 컴퓨팅이라 하고 SaaS는 사용자가 필요할 때 소프트웨어 기능을 이용하고 그만큼 비용을 지불하는 개념이다.
클라우드 컴퓨팅은 그리드 컴퓨팅과 SaaS를 합쳐놓은 형태이다. 언제 어디서나 응용 프로그램과 데이터를 자유롭게 사용할 수 있는 컴퓨팅 환경이다. 하드웨어를 포함한 시스템이 구름에 가려 보이지 않는 컴퓨팅 환경이라는 의미이다.
-->

6. API와 SDK를 비교하여 설명하시오.
: <!-- API는 카카오 맵 api와 같이 내가 원하는 응용 프로그램을 만들 때 기능을 빌려와서 사용하는 함수같은 그런 느낌이다.
SDK는 내가 원하는 기능을 빌려쓸 수 있을 뿐만 아니라 배포와 같은 기능까지도 가능하다. 예로써 안드로이드 스튜디오가 있다.-->
<!-- 
답) API는 응용 프로그램이 자신과 연관된 프로그램은 만들 수 있도록 제공하는 인터페이스이다.
...
: <!-- 단일형 구조 커널은 하나의 모듈에 커널의 모든 기능을 넣은 형태이다. C언어로 따진다면 main() 함수에 모든 기능을 넣은 형태로 생각하면 된다. 이렇게 된다면 모듈 간 통신에서 발생하는 지연이 발생하지 않아서 효율성있게 사용할 수 있지만 오류나 버그가 생겼을 때 찾기가 어렵다. -->
<!--
답) 단일형 구조 커널은 초창기 OS 구조로 커널의 핵심 기능을 구현한느 모듈들이 구분 없이 하나로 구성되어 있다.
모듈이 분리되지 않아서 모듈 간 통신 비용이 줄어들어 효과적인 운영이 가능하다. 그러나 모듈이 하나로 묶여있어 버그, 우류 처리가 어렵다.OS의 여러 기능이 서로 연결되어 있어 상호 의존성이 높아 기능상의 작은 결함이 시스템 전체로 확산될 수 있다. 다양한 환경의 시스템에 적용하기 어렵다. 단일형 구조에서는 수정이 어려워 이식성이 낮다. 현대의 OS는 크고 복잡해서 단일형으로 구현이 어렵다.
-->
8. 마이크로 구조 커널의 특징을 설명하시오.
: <!-- 마이크로 구조 커널은 최소한의 기능 프로세스 관리, 메모리 관리, IPC의 기능만을 가진 커널이다. 이렇게 최소한의 기능만을 가지다 보니까 사용자 영역에 구현된 기능이 많다. 사용자 영역에 구현된 기능과 데이터를 주고 받기 위해 IPC를 사용하여 통신한다.-->
<!--
답) 마이크로 구조 커널은 프로세스 관리, 메모리 관리, IPC 등 가장 기본적인 기능만 제공한다. 커널의 구조를 살펴보면 다른 커널에 비해 OS의 많은 부분이 사용자 영역에 구현되어 있다. 각 모듈은 세분화되어 존재하고 모듈 간 정보 교환은 IPC를 이용한다. 각 모듈은 독립적으로 작동하여 하나의 모듈이 실패해도 전체 OS가 멈추지 않는다. 또한 이식성이 뛰어나고 커널이 작아 CPU 용량이 작은 시스템에도 적용가능하다.
-->
