...
- 레벨별 캐시의 구조를 보여준다.
<img src="https://user-images.githubusercontent.com/78334910/148170920-40b3099b-1b45-4e17-93f7-bea2f7651060.PNG">
...
> 그러나 현재의 기술과 비용 문제로 쉽지않다. 그래서 개발사들은 CPU의 핵심 기능을 가진 코어를 여러 개 만들거나, 돋시에 실행가능한 명령의 개수를 늘리는 방법을 사용한다.

### 1 병렬 처리의 개념
- 동시에 여러 개의 명령을 처리하여 작업의 능률을 올리는 방식
- 코어가 하나인 CPU도 작동 가능하다. 볶음밥을 만드는데 40분이 걸리고 2개를 한 주방에서 만드는데 80분이 걸린다고 생각하자.
- 그런데 볶음밥을 세분화하면 4개의 과정이 나오는데 각 과정들은 10분씩 걸린다고 생각했을때 각 과정을 겹치지 않게 동시에 진행하면 시간이 많이 단축된다.
- 아래의 그림은 여러 개의 볶음밥을 동시에 만드는 과정이다.
![image](https://user-images.githubusercontent.com/78334910/148362960-788f009e-7303-45f4-bb9c-c514aace0e3a.png)

- 볶음밥 A 재료의 세척이 끝나 손질에 들어가면 볶음밥 B의 재료를 세척하고 볶음밥 B의 재료가 손질에 들어가면 볶음밥 C의 재료를 세척한다. 이러한 방식을 이용하면 120분이 걸리던 일도 60분이 걸리게 만든다.
- 한 주방에서 여러 개의 볶음밥을 동시에 조리하는 것은 CPU의 사양에 비유하면 하나의 코어에 여러 개의 스레드를 이용하는 방식과 같다. 병렬 처리에서는 이러한 방식을 파이프라인 기법이라고 부른다.
- 만약 한 주방에 2개의 조리 시설이 갖춰어져 있다면 2개의 요리를 동시에 할 수 있다. 이것을 슈퍼스칼라 기법이라고 부른다.
- 예로써 듀얼코어 CPU는 한 CPU에 작업을 처리하는 코어(조리 시설)가 2개 있고, 슈퍼스칼라 기법을 이용하여 2개의 작업을 동시에 처리할 수 있다.

< CPU 멀티스레드 >
- 스레드는 CPU의 작업 단위로, 한 번에 여러 개의 스레드를 처리하는 방식을 멀티스레드라고 한다.
- 스레드는 운영체제가 사용하는 프로그래밍 기법이다.

### 2 병렬 처리 시 고려사항
1. 상호 의존성이 없어야 병렬 처리가 가능하다.
  - 각 명령이 서로 독립적이고 앞의 결과가 뒤의 명령에 영향을 미치지 않아야 한다는 뜻이다.
  - 위 볶음밥을 만드는 그림을 보면 A는 채소를 손질하는 것인데 만약 B가 손질한 채소를 볶는 것이라면 A가 끝날 때까지 B는 작업을 시작할 수 없다.
  - 즉 앞뒤 단계 간에 상호 의존적인 명령어를 사용하면 병렬 처리를 할 수 없다.
2. 각 단계의 시간을 거의 일정하게 맞춰야 병렬 처리가 원만하게 이루어진다.
  - 앞에서 각 단계의 시간을 10분으로 했기에 병렬 처리가 원만하게 이루어진다. 만약 전부 시간이 다르다고 생각하면 아래 그림처럼 될 것이고 가장 긴 시간일 걸리는 단계에서 병목 현상이 발생한다. 이처럼 단계별 시간의 차이가 크면 병렬 처리의 효과가 떨어진다.
![image](https://user-images.githubusercontent.com/78334910/148364951-466b308e-d760-48cd-8522-24479ef2618c.png)

3. 전체 작업 시간을 몇 단계로 나눌지 잘 따져보아야 한다.
- 작업을 N개로 쪼갰을 때 N을 병렬 처리의 깊이라고 한다.
- 이론적으로는 N이 커질수록 성능이 높아질 것이다. 하지만 작업을 너무 많이 나누면 각 단계마다 작업을 이동하고 새로운 작업을 불러오는 데 시간이 너무 많이 걸려서 오히려 성능이 저하된다. 이러한 오버헤드를 고려하여 보통은 병렬 처리의 깊이를 10~20 w정도로 한다.

![image](https://user-images.githubusercontent.com/78334910/148369540-ca629b36-915b-44d9-ae0d-de268e6d7b2f.png)


### 3 병렬 처리 기법
- CPU 내에서는 명령어는 제어장치가 처리한다. 제어장치는 명령어를 가져와 해석한 후 실행하고 결과를 저장하는 과정을 반복한다.
- 이러한 과정 전체를 하나의 스레드라고 하며 스레드를 이루는 각 단계는 CPU의 클록과 연동되어 한 클록에 한 번씩 이루어진다.
- CPU는 다음과 같은 4개의 과정을 계속 반복하면서 명령어를 처리한다.
...
![image](https://user-images.githubusercontent.com/78334910/148369691-fbf57fb2-117e-4c1a-825f-5ddb8a464737.png)

- 병렬 처리 기법은 파이프라인과 슈퍼 스칼라 기법으로 나뉘고 이 외에도 여러 기법들이 있는데 각 기법의 특징을 자세히 알아보자

#### 3.1 파이프라인 기법
- CPU의 사용을 극대화 하기위해 명령을 겹쳐서 실행하는 방법으로 CPU의 사양과 연관지어 보면 하나의 코어에 여러 개의 스레드를 사용하는 것이다.
- 명령어를 여러 개의 단계로 분할한 후, 각 단계를 동시에 처리하는 하드웨어를 독립적으로 구성한다. (동작은 위에서 설명했음)
- 파이프라인의 위험이라고 여러 가지 문제가 있는데 크게 데이터 위험, 제어 위험, 구조적 위험으로 구분된다.
1. 데이터 위험
  - 데이터의 의존성으로 발생하는 문제
  - 데이터 A를 필요로 하는 두 번째 명령어는 앞의 명령어가 끝날 때까지 동시에 실행되어서는 안된다.
  - 파이프라인의 명령어 단계를 지연하여 해결한다.
![image](https://user-images.githubusercontent.com/78334910/148370307-a3f0b08d-c37c-47ae-adce-8b223bc938ef.png)

2. 제어 위험
  - 분기를 하는(if, goto 등) 명령에서 발생한다. 프로그램 카운터 값을 갑자기 변화시켜 발생하는 위험이다.
  - 일반적으로 모든 프로그램이 순차적으로 실행된다고 가정을 하는데 갑자기 goto문이 나와 다음 문자이 아닌 다른문장으로 간다면 현재 동시에 처리되는 명령어들이 쓸모없어진다.
  - 분기 예측이나 분기 지연 방법으로 해결한다.
![image](https://user-images.githubusercontent.com/78334910/148370653-0be790f7-81a7-41c7-84e5-b71adfe99b8f.png)

3. 구조 위험
  - 서로 다른 명령어가 같은 자원에 접근하려 할 때 발생하는 문제
  - 예로써 명령어 A가 레지스터 RX를 사용하고 있는데병령 처리되는 B도 레지스터 RX를 사용해야 하면 충돌이발생한다.
  - 이러한 구조 위험은 해결하기 어렵다.
![image](https://user-images.githubusercontent.com/78334910/148370849-55935421-3869-4423-8dbd-20c6896e29b6.png)

  - 인텔 계열의 CPU에서는 파이프라인 기법을 하이퍼 스레드라고 부른다. 이는 CPU에 여러 개의 작업을 동시에 진행할 수 있는 부가장치를 만들어 하나의 코어에서도 여러 개의 작업(스레드)이 동시에 이루어지게 하는 기법이다.

#### 3.2 슈퍼스칼라 기법
- 파이프라인을 처리할 수 있는 코어를 여러 개 구성하여 복수의 명령이 동시에 실행되도록 하는 기법
- 파이프라인과 동일하지만 조리 시설 즉 코어가 여러개여서 각 단계에서 진행되는 명령어가 2개라는 점이 다르다.
- 파이프라인과 마찬가지로 처리되는 명령어가 상호 의존성이 없어야 한다. 이를 위한 처리도 컴파일러에서 이루어지도록 조정해야 한다.
![image](https://user-images.githubusercontent.com/78334910/148371432-46e75385-b85b-483c-80e8-ddc271723ed4.png)

#### 3.3 슈퍼파이프라인 기법
- 파이프라인을 강화한 기법
- 기존의 파이프라인은 한 클록마다 하나의 명령어를 처리하지만 슈퍼파이프라인 기법에서는 파이프라인의 각 단계를 세분하여 한 클록 내에 여러 명령어를 처리할 수 있다.
![image](https://user-images.githubusercontent.com/78334910/148371925-629f9155-d4fb-48f3-869f-5f958886422d.png)

#### 3.4 슈퍼파이프라인 슈퍼스칼라 기법
- 앞에 것들을 모두 합쳐놓은 것이다.
- 슈퍼파이프라인을 여러 코어로 돌리는 기법이다.
![image](https://user-images.githubusercontent.com/78334910/148372154-700ac847-686e-4062-8892-7825c04fd9d3.png)

#### 3.5 VLIW 기법
- 지금까지 설명한 병렬 처리 기법과 완전히 다르다.
- 앞의 병렬 처리 기법들은 하드웨어적인 방법이지만, VLIW 기법은 CPU가 병렬 처리를 지원하지 않을 경우 소프트웨어적으로 병령 처리를 하는 방법이다.
- 동시에 수행할 수 있는 명령어들을 컴파일러가 추출하고 하나의 명령어로 압축하여 실행한다.
- CPU가 병렬 처리를 지우너하지 않을 때 사용하는 방법이므로 앞의 병렬 처리 기법들에 비해 동시에 처리하는 명령어의 개수가 적다.
- 또한 앞의 병렬 처리 기법들은 명령어 실행 시 병렬 처리가 이루어지지만 VLIW는 컴파일 시 병렬 처리가 이루어진다.


## 05 [심화학습] 무어의 법칙과 암달의 법칙
### 1 무어의 법칙
- 고든 무어는 CPU의 속도는 24개월마다 2배 빨라진다는 무어의 법칙을 주장
- 초기에는 적용되었지만 지금은 그렇지 않다.
- CPU는 자체 발열 문제로 속도를 5GHz 이상 높이기 어려움 -> 멀티코어를 장착
- 듀얼코어는 CPU 2개를 사용하는 효과를 얻을 수 있다. 또한 멀티 스레드도 많이 사용되고 있다.
### 2 암달의 법칙
- 진 암달은 컴퓨터 시스템의 일부를 개선할 때 전체 시스템에 미치는 영향과의 관계를 수식으로 나타낸 암달의 법칙을 만들었다.
- 이 법칙에 따르면 주변장치의 향상없이 CPU의 속도를 2GHz에서 4GHz로 늘려도 컴퓨터의 성능이 2배 빨라지지 않는다.
- 멀티코어에도 적용되는데, 코어가 하나인 싱글코어 대신 듀얼코어를 사용하더라도 CPU 내 다른 부품의 병목 현상으로 CPU의 성능이 2배가 되지 않는다.


## [ 연습 문제 ] 
1. CPU의 구성에 대해 설명하시오.
: 산술논리 연산 장치 : CPU에서 데이터를 연산하는 장치
레지스터 : CPU 내에 데이터를 임시로 보관하는 장치 
제어 장치 : CPU에서 작업 지시를 하는 장치

2. 폰노이만 구조의 가장 중요한 특징을 설명하시오.
: 모든 프로그램은 메모리에 올라와야 실행할 수 있다.

3. 버스의 종류를 나열하시오.
: 제어 버스 : 다음 작업을 지시하는 제어 신호
주소 버스 : 메모리의 위치 정보를 알려주는 주소 신호
데이터 버스 : 처리할 데이터 신호

4. 단방향 버스에 대해 설명하시오.
: 데이터가 한 방향으로 이동하는 버스이며 주소 버스가 있다.

5. 다음에 실행할 명령어의 주소를 가지고 있는 레지스터는 무엇인가?
: 프로그램 카운터 (Program Counter, PC) : 다음에 실행할 명령어의 주소를 기억하고 있다가 제어장치에 알려준다.
 명령어 레지스터 (Instruction Register, IR) : 현재 실행 중인 명령어를 저장한다.
 메모리 주소 레지스터 (Memory Address Register) : 메모리에서 데이터를 가져오거나 반대로 메모리에서 데이터를 보낼 때 주소를 지정하기 위해 사용한다.
 메모리 버퍼 레지스터 (Memory Buffer Register) : 메모리에서 가져온 데이터나 메모리로 옮겨 갈 데이터를 임시로 저장한다. MBR은 MAR과 항상 함께 동작한다.

6. 다음에 실행할 명령어를 보관하는 레지스터는 무엇인가?
: 명령어 레지스터 (Instruction Register, IR) : 명령 계수기가 지정하는 번지에 기억되어 있는 명령어를 호출해서 해독하기 위해 명령어를 잠시 보관해 두는 특수 목적 레지스터이다. 현재 실행되거나 디코딩되고 있는 명령을 보관하고 있다.

7. 메모리 주소를 보관하는 레지스터는 무엇인가?
: 메모리 주소 레지스터 (Memory Address Register) : 메모리에서 데이터를 가져오가나 반대로 메모리에서 데이터를 보낼 때 주소를 지정하기 위해 사용한다.

8. 메모리에 저장할 데이터나 메모리에서 가져온 데이터를 임시로 보관하는 레지스터는 무엇인가?
: 메모리 버퍼 레지스터 (Memory Buffer Register) : 메모리에서 가져온 데이터나 메모리로 옮겨갈 데이터를 임시로 저장한다. MBR은 MAR과 항상 함께 동작한다.
...
: 경계 레지스터 (bound register) : 현재 진행 중인 작업의 메모리 시작 주소를 저장하고 있음.
  한계 레지스터 (limit register) : 현재 진행 중인 작업이 차지하고 있는 메모리의 크기, 즉 마지막 주소까지의 차이를 저장하고 있음.

12. 속도 차이가 나는 두 장치 사이에서 속도를 완화하는 장치를 무엇이라고 통칭하는가?
: 버퍼

13. 응용프로그램과 프린터 사이에서 속도 차이를 완환하는 소프트웨어를 무엇이라고 하는가?
: 스풀러
...
17. CPU 도움 없이도 메모리를 사용할 수 있도록 입출력 관리자에게 주는 권한을 무엇이라 하는가?
: 메모리 직접 접근 권한

18. CPU와 입출력 관리자가 서로 다른 메모리 영역을 사용하는 기법을 무엇이라고 하는가?
: 메모리 매핑 입출력 (Memory Mapping I/O, MMIO) : CPU가 사용하는 메모리 공간과 직접 메모리 접근을 통해 들어오거나 나가는 데이터를 위한 공간을 분리하는 것이다.
...
: 클록은 CPU의 속도와 관련된 단위이다. 클록이 일정 간격으로 틱을 만들면 거기에 맞춰 CPU 안의 모든구성 부품이 일을 한다. 버스에는 여러 개의 부품이 연결되어 있는데, 메인보드의 클록이 틱을 보낼 때마다 데이터를 보내거나 받는다.

2. 프로그램 카운터 레지스터의 역할을 설명하시오.
: 다음 실행할 명령어의 주소를 기억하고 있다가 제어장치에 알려준다.

3. 제어 버스, 주소 버스, 데이터 버스에 대해 설명하시오.
: 제어 버스는 제어 장치와 연결고 양방향 버스이다. 다음에 어떤 작업을 할지 지시하는 제어 신호가 오고 간다.
주소 버스는 메모리의 데이터를읽거나 쓸 때 어느 위치에서 작업할 것인지를 알려주는 위치 정보(주소)가 오고 간다. 메모리 주소 레지스터와 연결되어있고 단방향 버스이다.
데이터 버스는 데이터가 이동하는 버스이고 메모리 버퍼 레지스터와 연결되어 있으며 양방향이다.

4. 휘발성 메모리와 비휘발성 메모리를 비교하여 설명하시오.
: 휘발성 메모리는 DRAM, SRAM이 있다. DRAM은 저장된 데이터가 일정 시간이 지나면 사라지므로 일정 시간마다 다시 재생시켜야 한다. SRAM은 전력이 공급되는 동안에는 데이터를 보관할 수 있어 재생할필요가 없다. 가격이 비싸지만 속도가 빠르다. 일반적으로 메인 메모리는 DRAM을 사용하고 캐시 같은 고속 메모리에는 SRAM을 사용한다. SDRAM은 클록틱이 발생할 때마다 데이터를 저장하는 동기 DRAM이다.
비휘발성 메모리는 플래시 메모리, FRAM, PRAM 등이 있다. 플래시 메모리는 USB같이 전력이 없어도 데이터를 보관할 수 있는 저장장치로 많이 사용된다. SSD는 하드디스크를 대신하도록 만들어졌다. 가격이 비싸지만 빠른 데이터 접근 속도,저전력, 내구성 때문에 많이사용된다. 전력이 끊겨도 내용이 남아있는 메모리이다.

5. DDR SDRAM에 대해 설명하시오.
: SDRAM의 CPU와의 속도 차이를 완화하기 위해 만들어진 메모리이다. SDRAM의 대역폭을 늘려 데이터 입출력 속도를 빠르게 한 것이 DDR(Double Data Rate)  SDRAM이다.

6. 스풀에 대해 설명하시오.
: 스풀은 CPU와 입출력장치가 독립적으로 동작하도록 고안된 소프트웨어적인 버퍼로, 대표적으로 프린터에 이용된다. 인쇄할 내용을 하드디스크의 스풀러 공간에 저장하고 CPU는 다른 작업이 가능하다. 스풀러는 일종의 버퍼지만 기존의 버퍼와 다른 점이 있다. 프로그램이 버퍼를 공유하지만 스풀러는 프로그램 간에 배타적이다.

7. 즉시 쓰기 방식의 캐시와 지연 쓰기 방식의 캐시를 비교하여 설명하시오.
: 캐시는 메모리에서 실행될 것 같은 데이터를 미리 가져오는 방식의 메모리인데 만약 캐시에서 데이터가 수정이 되면 땡겨온 메인 메모리의 데이터도 수정이 되야한다. 이 과정에서 즉시 쓰기 방식의 캐시는 데이터가 수정된 그 시점에 바로바로 메모리의 데이터도 수정하는 방식이고 빈번한 데이터 전송으로 성능이 느려진다는 단점과 메모리에 최신 값이 항상 유지되어 급작스러운 정전에도 데이터 손실이 없다는 장점이 있다. 지연 쓰기 방식의 캐시는 주기적으로 데이터를 수정하여 일정 기간동안 쌓아놓았다가 처리하고 데이터 전송이 줄어들어 시스템의 성능을 향상시킨다는 장점과 메모리와 캐시된 데이터 사이의 불일치가 발생할 수 있다는 단점이 있다.

8. 인터럽트 번호를 사용하는 이유를 설명하시오.
: 과거에는 입출력 장치를 마우스, 키보드, 모니터와 같이 적은 수의 입출력 장치를 사용했다. 그러나 최근에는 프린터, 스캐너 등의 여러 입출력장치들이 생겼고 이 장치들의 인터럽트가 누구의 인터럽트인지의 구별이 필요하게 되어 인터럽트 번호를 사용하게 되었다.

9. 메모리 직접 접근(DMA)에 대해 설명하시오.
: 입출력 장치로 부터 인터럽트가 입출력 관리자에게 오면 입출력 관리자는 이 내용에 따라서 CPU 대신 메모리에 접근을 해야하는 상황이 생긴다. 하지만 메모리에는 CPU만 접근할 수 있는 권한이 있어서 이것을 해결하고자 입출력 관리자가 CPU의 허락없이 메모리에 접근할 수 있는 권한을 주기 위한 기능이다.

10. 메모리 매핑 입출력에 대해 설명하시오
: 직접 메모리 접근은 인터럽트 방식의 시스템을 구성하느데 필수 요소지만 이걸 사용하면 메모리가 복잡해진다. 직접 메모리 접근을 통해 들어온 데이터를 메모리에 아무렇게나 둔다면 CPU가 사용하는 데이터와 섞여 관리하기 어려울 것이므로 이를 막기 위해 메모리를 나누어 사용하는 방법이 도입되었다. CPU가 사용하는 메모리 공간과 직접 메모리 접근을 통해 들어오거나 나가는 데이터를 위한 공간을 분리하는 것이다. 이렇게 메모리의 일정 공간을 입출력에 할당하는 기법을 메모리 매핑 입출력이라고 한다.
